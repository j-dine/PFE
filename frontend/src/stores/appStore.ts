  ...
  state: () => {
    ...
    return {
      ...
      roles: [
        ...
        { key: 'rh', label: 'Service RH' },
        { key: 'juridique', label: 'Service Juridique' },
        { key: 'technique', label: 'Service Technique' },
      ],
      usersMap: {
        ...
        rh: { name: 'Fatima Zerrouki', init: 'FZ', color: '#059669', roleLabel: 'Chef Service RH' },
        juridique: { name: 'Maître Karim Boucetta', init: 'KB', color: '#7c3aed', roleLabel: 'Conseiller Juridique' },
        technique: { name: 'Ing. Samir Bensalem', init: 'SB', color: '#d97706', roleLabel: 'Ingénieur Principal' },
      ...
      navMap: {
        ...
        dg: [
          {
            title: 'Direction Générale', items: [
              { key: 'dg-dashboard', label: 'Tableau de bord', icon: '<rect x="3" y="3" width="7" height="7"/>', badge: '' },
              { key: 'dg-valider', label: 'Valider dossier', badge: '24', badgeClass: 'blue', icon: '<path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/>' },
              { key: 'dg-rapports', label: 'Générer rapport', icon: '<line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/>' },
            ]
          },
        ],
        rh: [
          {
            title: 'Service RH', items: [
              { key: 'rh-dashboard', label: 'Tableau de bord', icon: '<rect x="3" y="3" width="7" height="7"/>', badge: '' },
              { key: 'rh-traiter', label: 'Traiter dossier', icon: '<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>', badge: '', badgeClass: 'amber' },
              { key: 'rh-nouveau', label: 'Nouvelle demande', icon: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="15" y2="15"/>' },
            ]
          },
        ],
        juridique: [
          {
            title: 'Service Juridique', items: [
              { key: 'juridique-dashboard', label: 'Tableau de bord', icon: '<rect x="3" y="3" width="7" height="7"/>', badge: '' },
              { key: 'juridique-avis', label: 'Rédiger avis', icon: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>' },
              { key: 'juridique-contrats', label: 'Contrats', icon: '<path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2"/>' },
            ]
          },
        ],
        technique: [
          {
            title: 'Service Technique', items: [
              { key: 'technique-dashboard', label: 'Tableau de bord', icon: '<rect x="3" y="3" width="7" height="7"/>', badge: '' },
              { key: 'technique-rapport', label: 'Rapport technique', icon: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>' },
              { key: 'technique-demande', label: 'Demande technique', icon: '<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/>' },
            ]
          },
        ],
        af: [
  ...
  actions: {
      ...
      try {
        ...
        if (Array.isArray(apiUsers)) {
          const roleToUi = (roles: any): { label: string; color: string } => {
            ...
            if (roleName.includes('DG')) return { label: 'Directeur Général', color: '#1d4ed8' }
            if (roleName.includes('RH')) return { label: 'Service RH', color: '#059669' }
            if (roleName.includes('JURIDIQUE')) return { label: 'Service Juridique', color: '#7c3aed' }
            if (roleName.includes('TECHNIQUE')) return { label: 'Service Technique', color: '#d97706' }
    ...
    async createNewUser(payload: any) {
      ...
      const roleName = (() => {
        ...
        if (v.includes('directeur général') || v.includes('dg')) return 'ROLE_DG'
        if (v.includes('rh')) return 'ROLE_RH'
        if (v.includes('juridique')) return 'ROLE_JURIDIQUE'
        if (v.includes('technique')) return 'ROLE_TECHNIQUE'
        ...
        v.includes('admin') ? 'ROLE_ADMIN'
          : v.includes('responsable') ? 'ROLE_RESPONSABLE'
            : v.includes('financier') ? 'ROLE_AGENT_FINANCIER'
              : v.includes('directeur général') || v.includes('dg') ? 'ROLE_DG'
                : v.includes('rh') ? 'ROLE_RH'
                  : v.includes('juridique') ? 'ROLE_JURIDIQUE'
                    : v.includes('technique') ? 'ROLE_TECHNIQUE'
    ...
    async deleteUser(userId: number | string) {
      try {
        ...
        await this.fetchUsers()
        this.addToast('success', 'Utilisateur supprimé avec succès !')
      } catch (error) {
        this.addToast('error', 'Erreur lors de la suppression de l\'utilisateur.')
        throw error
      }
    },
