...
const filteredAdminUsers = computed(() => {
  const query = String(store.userSearch || '').trim().toLowerCase()
  return adminUsers.value.filter((u: any) => {
    const matchesSearch = !query || String((u.name || u.email || u.role) || '').toLowerCase().includes(query)
    const matchesRole = selectedRole.value === 'Tous rôles' || String(u.role || '') === selectedRole.value
    return matchesSearch && matchesRole
  })
})
const statsDistrib = computed(() => store.statsDistrib)
const statsCounters = computed(() => store.statsCounters)
const roleOptions = computed(() => [{ key: 'all', label: 'Tous rôles' }, ...store.roles])
const selectedRole = ref('Tous rôles')
      ...
      <div class="card">
        <div class="card-head">
          ...
          <div style="display:flex;gap:8px">
            ...
            <select class="form-select" style="width:160px;font-size:11px;padding:6px 10px" v-model="selectedRole">
              <option v-for="option in roleOptions" :key="option.key" :value="option.label">{{ option.label }}</option>
            </select>
          </div>
        </div>
        <table class="tbl">
            ...
            <tr v-for="u in filteredAdminUsers" :key="u.id">
              ...
              <td>
                <div style="display:flex;gap:5px">
                  ...
                  <button class="btn btn-soft-danger btn-sm" style="background:var(--red-s);color:var(--red)" @click="confirmDeleteUser(u.id)">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:12px;height:12px;margin-right:0"><path d="M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2M10 11v6M14 11v6"/></svg>
                    Supprimer
                  </button>
                </div>
              </td>
